...
document.getElementById('btnPlay').addEventListener('click', function () {      
    ...
    const answerFieldNumber = document.getElementById('answerFieldNumber');

    let u = ['', 'один', 'два', 'три', 'четыре', 'пять', 'шесть', 'семь', 'восемь', 'девять'];
    let t = ['', 'десять', 'одинадцать', 'двенадцать', 'тринадцать', 'четырнадцать', 'пятнадцать', 'шестнадцать', 'семнадцать', 'восемнадцать', 'девятнадцать'];
    let d = ['', 'двадцать', 'тридцать', 'сорок', 'пятьдесят', 'шестьдесят', 'семьдесят', 'восемьдесят', 'девяносто'];
    let h = ['', 'сто', 'двести', 'триста', 'четыреста', 'пятьсот', 'шестьсот', 'семьсот', 'восемьсот', 'девятьсот'];
    ...
    function numberToText() { 
        ...
        if (number <= 9) {
            return u[Math.floor(Math.abs(number) / 1)];
        }

        if (number > 9 && number < 20) {
            return t[Math.floor(number / 10 + number % 10)];
        }

        if (number >= 20 && number <= 99) {
            return d[(Math.floor(number / 10)) - 1] + " " + u[Math.floor(number % 10)];
        }

        if (number >= 100 && number <= 999) {
            return h[Math.floor(number / 100)] + " " + numberToTextHundreds();
        }
    }

    function numberToTextHundreds() { 
        let utd = Math.abs(answerNumber) % 100;

        if (utd <= 9) {
            return u[Math.floor(utd / 1)];
        }

        if (utd > 9 && utd < 20) {
            return t[(Math.floor(utd / 10)) + (utd % 10)];
        }

        if (utd >= 20 && utd <= 99) {
            return d[(Math.floor(utd / 10)) - 1] + " " + u[Math.floor(utd % 10)];
        }
    }
    ...
    document.getElementById('btnEqual').addEventListener('click', function () { 
        ...
        if (gameRun) {
            ...
            switch (phraseRandom) {
                case 0:
                    answerPhrase = `Моя угадайка превосходна\n\u{1F60E}`
                    break;

                case 1:
                    answerPhrase = `Восхитительно! \n\u{1F60E}`
                    break;

                case 2:
                    answerPhrase = `Я не подражаем!\n\u{1F973}`
                    break;

                case 3:
                    answerPhrase = `Победа у меня в кормане!\n\u{1F929}`
                    break;
            }
            answerField.innerText = answerPhrase;
